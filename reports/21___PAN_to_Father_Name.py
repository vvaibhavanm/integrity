# pages/page_pan_to_fname_verification.py
...
pan_to_fname_VERIFICATION_URL = "https://api.invincibleocean.com/invincible/panPro"
...
def verify_pan_to_fname(pan_to_fname: str, clientid: str, secretkey:str) -> Dict[str, Any]:
    if (not clientid) or (not secretkey):
        log_error("Invincible auth is missing.")
        return {}

    url = pan_to_fname_VERIFICATION_URL
    ...
    try:
        ...
        data = response.json()
        print(data)
        log_info(f"PAN to fname Verification successful for pan: '{pan_to_fname}'")
        return data

    except requests.exceptions.HTTPError as http_err:
        log_error(f"HTTP error during PAN to fname verification for pan '{pan_to_fname}': {http_err}")
    except requests.exceptions.ConnectionError as conn_err:
        log_error(f"Connection error during PAN to fname verification for pan '{pan_to_fname}': {conn_err}")
    except requests.exceptions.Timeout as timeout_err:
        log_error(f"Timeout error during PAN to fname verification for pan '{pan_to_fname}': {timeout_err}")
    except requests.exceptions.RequestException as req_err:
        log_error(f"Request exception during PAN to fname verification for pan '{pan_to_fname}': {req_err}")
    except ValueError as json_err:
        log_error(f"JSON decoding failed for PAN to fname verification response of pan '{pan_to_fname}': {json_err}")
    except Exception as e:
        log_error(f"An unexpected error occurred during PAN to fname verification for pan '{pan_to_fname}': {e}")

    return {}

@st.cache_data
def cache_pan_to_fname_verification(pan: str , clientid: str, secretkey:str) -> Dict[str, Any]:
    return verify_pan_to_fname(pan, clientid, secretkey)
...
def process_pan_to_fname_verification(file: BytesIO, file_type: str, clientid: str, secretkey:str) -> Optional[pd.DataFrame]:
    try:
        if file_type == "csv":
            df = pd.read_csv(file)
        ...
        for idx, row in df.iterrows():
            ...
            # Verify pan
            verification_result = cache_pan_to_fname_verification(pan, clientid, secretkey)
...
def run():
    st.header("PAN to fname")
    st.write("PAN to fname verification is a process to verify the fname details using the PAN number.")
    ...
    uploaded_file = st.file_uploader(
        "Upload your PAN to fname file (CSV or XLSX).",
        type=["csv", "xlsx"],
        key="pan_to_fname_verification"
    )

    if uploaded_file:
        file_type = uploaded_file.name.split('.')[-1]
        if st.button("Run PAN to fname", key="run_pan_to_fname_verification"):
            with st.spinner("Processing PAN to fname..."):
                processed_df = process_pan_to_fname_verification(
                    file=uploaded_file,
                    file_type=file_type,
            ...
            if processed_df is not None:
                st.success("PAN to fname completed successfully.")
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name="pan_to_fname_verification_results.csv",
                    mime='text/csv',
                )
